...
object particlesGoUp: ApplicationAdapter("Particles Go Up",800,600){
    override fun init() {
        ...
        val maxVel = 1000f
        fun rColor() = Color.getHSBColor(Math.random().toFloat(),Math.random().toFloat(),Math.random().toFloat())
        repeat(200){
    ...
    // No need to include anything else for simple scenes and hacking at random ideas, but do it anyway if ya like.
}
