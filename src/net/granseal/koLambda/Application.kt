...
abstract class Application(val title: String, val width: Int, val height: Int, undecorated: Boolean = false) {
    private val timer = Timer()
    private val frameTimer = Timer()
    val logger = Logger.getLogger("")!!
    var started = false
    private var root = JFrame(title).apply { isUndecorated = undecorated }
    ...
    fun start(){
        ...
        while(started){
            ...
            if (framesTimer >= 1){
                framesTimer -= 1
                root.title = "$title (FPS $frames) $VERSION"
                frames = 0
            }
        }
